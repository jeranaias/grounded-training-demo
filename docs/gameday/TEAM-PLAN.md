...
Five people, five lanes, one intake loop. Everyone has a clear job that maps to a real part
...
|---|---|---|
| **Jesse** | Lead · grounding & integration | Own Anchor + the doctrine seam + wiring the 12 soldiers into the app |
...
| **Morgan** | Content, corpus & SME review | Source/screen doctrine, run ingest, own rubric/assessment review + the guardrails |
...
flowchart LR
  ...
  B --> TR["Triage<br/>(Jesse + White + Thompson)"]
  TR -->|frontend| M["McDonald<br/>builds the UI change"]
  TR -->|backend / data / grounding| BE["Jesse + White"]
  TR -->|content / rubric / corpus| MO["Morgan"]
  M --> R["Review + merge"]
  BE --> R
  MO --> R
  R --> T
...
## Jesse — Lead · grounding & integration
**Owns:** Anchor + the Orin, the doctrine adapter (`lib/doctrine.js` / `DOCTRINE_BASE_URL`), the
grounded pipeline (Author/Deliver/Improve), env/secrets, offline verification, and the final
architecture call with White.
**Gameday tasks:**
...
**Reads:** the whole `../` stack, esp. `02-architecture`, `04-grounding-and-anchor`, `05-arsenal-contracts`, `08-build-guide`.
...
agreed product direction into real, on-brand UI. This is the "make it real" lane; Jesse/White pair
...
4. Workflow: branch → change → `npm run dev` to see it → push → Jesse/White review. Ask early, commit often.
...
## Morgan — Content, corpus & SME review
**Owns:** what the platform teaches, and the guardrail that keeps it honest. Standards/compliance lane.
**Gameday tasks:**
...
- Own **rubric + assessment review**: feed real T&R standards to `rubricon`, and ratify generated items (approve/edit/reject) — this is the "instructor owns the truth" guardrail made real.
- Own the submission package + decision log + the honest scorecard (COLD-BORE.md → HONESTY).
**Reads:** `../07-instructor-loop.md` (review/rubrics/AAR), `../04-grounding-and-anchor.md`, `../05-arsenal-contracts.md` (quarry/rubricon/cartridge), `../03-data-model.md` (Item/citation/status).

---

## Shared rhythm
- **Each morning:** Jesse runs `ops/orin-check.sh` (engine green); quick triage of Thompson's overnight issues; assign lanes.
- **Contracts before code:** backend (White/Jesse) and frontend (McDonald) agree the API/props shape for a screen *before* building it — the design system + data model are the shared language.
